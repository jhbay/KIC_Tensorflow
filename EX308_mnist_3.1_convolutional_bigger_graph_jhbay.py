...
## convolution HL -> FCN(full conn. layer)
Y4 = tf.nn.relu(tf.matmul(YY, W4) + B4)
Y4drop = tf.nn.dropout(Y4, pkeep)
Ylogits = tf.matmul(Y4drop, W5) + B5
...
for i in range(1000 + 1) :
    ...
    if i % 10 == 0 : 
        ...
        a, c = sess.run([accuracy, cross_entropy], feed_dict={X: mnist.test.images, Y_: mnist.test.labels, pkeep: 1.0, step: i})
        print("testing* : ",i, ' accuracy = ', '{:7.4f}'.format(a), ' loss = ', c)
        test_acc_list.append(a)
        test_loss_list.append(c)

    # the backpropagation training step
    sess.run(train_step, {X: batch_X, Y_: batch_Y, step: i, pkeep:0.75, step: i, })
